...
class Controller:
    ...
    def _get_main_menu_src(self):
        ...
        last_show_queries = lastvisited.LastVisited().get_last_show_visited()
        if last_show_queries:
            (last_anime_visited_title, queries) = main_menu[0]
            title = '%s - %s' % (last_show_queries.get('full_title'), last_anime_visited_title)
    ...
    def show_media_list(self):
        ...
        lastvisited.LastVisited().update_last_show_visited()
    ...
    def choose_quality_play_video(self):
        ...
        quality_player.determine_quality()
        quality_player.play()
    ...
    def show_bookmark_list(self):
        if not account.Account().is_logged_in():
            account.Account().log_in_saved()            		        
        if account.Account().is_logged_in():
